...
function M.setup()
  local override = require("setup.options").hl_override
  local theme = require("setup.colors").get().base_16
  vim.opt.bg = require("setup.colors").get().type
  local highlights = {}
  local hl_files = vim.fn.stdpath("config") .. "/lua/layers/ui/integrations"

  for _, file in ipairs(vim.fn.readdir(hl_files)) do
    local pkg_name = "setup.integrations." .. vim.fn.fnamemodify(file, ":r")
...
function M.get_lualine_theme()
  local colors = require("setup.colors").get().base_30
  local theme = {
    normal = {
